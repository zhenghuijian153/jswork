(function () {
    var JMS = function (id,rowCount,colCount,minLandMineCount,maxLandMineCount) {
        if(!(this instanceof JMS))
            return new JMS(id,rowCount,minLandMineCount,minLandMineCount,maxLandMineCount)
        this.doc =document
        this.table = this.doc.getElementById(id)
        this.cells = this.table.getElementsByClassName("td")
    ...
    JMS.prototype={
        ...
        drawMap: function () {
            var tds =[]
            if(window.ActiveXObject&&parseInt(navigator.userAgent.match(/msie([\d.]+)/i)[i])<8){
